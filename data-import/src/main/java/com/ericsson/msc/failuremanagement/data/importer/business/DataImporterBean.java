...
public class DataImporterBean {
    ...
    @Inject
    private EventCauseJPA eventCauseJPA;
    @Inject
    private FailureClassJPA failureClassJPA;
    @Inject
    private FailureTraceJPA failureTraceJPA;
    @Inject
    private UserEquipmentJPA userEquipmentJPA;
    @EJB
    private ErrorLogWriterBean errorLogWriterService;
    @Inject
    ...
    public void importSpreadsheet(HSSFWorkbook excelWorkbook) {
        ...
        Collection<EventCauseEntity> existingEventCauseCollection = eventCauseJPA.getAllEventCauses();
        for (EventCauseEntity evCa : existingEventCauseCollection) {
            eventCauseHashMap.put(evCa.getCauseCodeEventIdCK().hashCode(), evCa);
        }
        Collection<FailureClassEntity> existingFailureClassCollection = failureClassJPA.getAllFailureClasses();
        for (FailureClassEntity fail : existingFailureClassCollection) {
            failureClassHashMap.put(fail.getFailureClass(), fail);
        }
        Collection<UserEquipmentEntity> existingUserEquipmentCollection = userEquipmentJPA.getAllUserEquipment();
        for (UserEquipmentEntity ue : existingUserEquipmentCollection) {
            userEquipmentHashMap.put(ue.getTypeAllocationCode(), ue);
        }
        Collection<CountryCodeNetworkCodeEntity> existingCountryCodeNetworkCodeCollection = countryCodeNetworkCodeJPA.getAllCountryCodeNetworkCodes();
        ...
        Collection<CountryEntity> existingCountryCollection = countryJPA.getAllCountries();
    ...
    private void readBaseDataSheet(Workbook excelWorkbook) {
        ...
        Long initialPKValue = failureTraceJPA.getTotalNumberOfEntries() + 1;
        ...
        failureTraceJPA.batchInsertFailureTrace(failureTraceCollectionToFlush);
    }
    ...
    private void readEventCauseDataSheet(Workbook excelWorkbook) {
        ...
        eventCauseJPA.batchInsertEventCause(eventCauseCollection);
    }
    ...
    private void readFailureClassDataSheet(Workbook excelWorkbook) {
        ...
        failureClassJPA.batchInsertFailureClasses(failureClassCollection);
    }
    ...
    private void readUserEquipmentDataSheet(Workbook excelWorkbook) {
        ...
        userEquipmentJPA.batchInsertUserEquipment(userEquipmentCollection);
    }
    ...
    private void readOperatorDataSheet(Workbook excelWorkbook) {
        ...
        countryCodeNetworkCodeJPA.batchInsertCountryCodeNetworkCode(countryCodeNetworkCodeCollection);
    }
